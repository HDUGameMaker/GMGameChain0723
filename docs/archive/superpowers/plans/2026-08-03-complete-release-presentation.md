...
**Files:** Create `src/core/SaveEnvelope.js`, `src/ui/panels/save-recovery-panel.js`, `test/node/save-v9-envelope.test.mjs`, `test/browser-save-recovery.spec.js`; modify `src/core/SaveManager.js`, `src/main.js`, `src/ui/PopupManager.js`, `test/node/save-v7-migration.test.mjs`, `test/node/save-v8-migration.test.mjs`, `test/node/integration-events-save.test.mjs`.
...
- [ ] **Step 3: Implement** Keep `SaveEnvelope.js` pure (`canonicalizePayload`, `sha256`, `createEnvelopeRecord`, `verifyEnvelopeRecord`); expose and test only the five static SaveManager methods. The envelope contains exact fields `format`, `envelopeVersion`, `payloadVersion`, `buildId`, positive integer `sequence`, `payload`, lowercase 64-hex SHA-256 `checksum`, cloned `migrationHistory` and `recoveryReport`; canonicalization sorts object keys recursively and rejects cycles, non-finite numbers, functions and unsupported values. SaveManager delegates to these helpers, validates the newly created envelope before storage, and rotates verified records in one IndexedDB transaction: previous rollback to emergency, previous primary to rollback, new envelope to primary; localStorage keeps the latest verified emergency copy. `chooseRecovery` verifies in strict `primary → rollback → emergency → import` order and returns `{source,envelope,payload,warnings}` with stable warning codes for rejected candidates. `load()` remains a compatibility wrapper over `loadRecoverable()`.

  Migration stays synchronous: v5–v7 follow the existing chain into v8, then v8 becomes v9 with `world: { schemaVersion: 1, source: 'legacy_static', mapId: 'base_map_v1' }`, `ownerId: 'player'` on legacy armies, off-map occupied colonies marked `legacyOffmap: true`, preserved legacy era-star ledger, and canonical `commerce`/`armyState` domain payloads. v9 removes duplicate top-level `armies`, `availableUnits`, `tradeRoutes` and `factions`; `main.js` saves/restores only the canonical domain keys. Unsupported/corrupt inputs fail closed without mutating the caller. The recovery panel reports the selected source and warnings after bootstrap but before normal interaction; reset clears all three IndexedDB records and local emergency data. Never use a fake digest.
- [ ] **Step 4: GREEN** Run: `node --test test/node/save-v9-envelope.test.mjs test/node/save-v7-migration.test.mjs test/node/save-v8-migration.test.mjs test/node/integration-events-save.test.mjs && npx playwright test -c test test/browser-save-recovery.spec.js`; Expected: PASS. `-c test` intentionally bypasses the repository's legacy root `testMatch` without changing global Playwright configuration; this spec selects installed Edge at file scope.
- [ ] **Step 5: Commit** `git add src/core/SaveEnvelope.js src/ui/panels/save-recovery-panel.js test/node/save-v9-envelope.test.mjs test/browser-save-recovery.spec.js src/core/SaveManager.js src/main.js src/ui/PopupManager.js test/node/save-v7-migration.test.mjs test/node/save-v8-migration.test.mjs test/node/integration-events-save.test.mjs; git commit -m "feat: add v9 save recovery"`
