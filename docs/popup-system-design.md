...
所有"打开弹窗"请求（`pm.open()`）统一进入**待显示队列**，串行展示：

```
open('daily_settlement')   → 队列: [settlement]            → 立即展示（当前无弹窗）
open('feature_unlock')     → 队列: [settlement, unlock]    → 等待 settlement 关闭
close()                    → settlement 关闭 → 队列排空     → 展示 unlock
```

- 多个来源（每日结算 / 赫斯提亚登场 / 奢侈品解锁 / 战斗预览……）同时请求弹窗时，**后到的请求不再覆盖已展示的弹窗**，而是等待其关闭后按入队顺序展示
- `pm.push()` 仍是弹窗内部的导航栈（钻取/返回），不受待显示队列影响
- 关闭流程中 `popupClosed` 回调里新发起的请求先入队，待关闭完成后再统一排空，保证暂停/恢复顺序确定
- 阻塞弹窗连续展示时保持暂停、不重复计数（`_pauseCount` 只在 0/1 之间变化），避免 BGM 淡入淡出抖动
- 不可关闭面板（`game_over` / `hestia_arrival` / `nonClosable`）只拦截玩家操作；面板自身流程（如赫斯提亚剧情结束时链入下一个弹窗）通过 `pm.close({ force: true })` 强制关闭
...
| 方法 | 用途 | 使用场景 |
|------|------|---------|
| `pm.open(type, data)` | 打开面板（入队；当前无弹窗立即展示，有弹窗则等待其关闭后按序展示） | 从外部入口打开（建筑点击、菜单、每日结算、赫斯提亚登场、战斗预览……） |
| `pm.push(type, data)` | 压入子面板（保留当前层，可返回） | 从面板内进入子详情页（如物品详情、训练→兵种研发） |
| `pm.pop()` | 返回上一层 | 点击返回按钮，栈空则关闭弹窗 |
| `pm.close({ force })` | 彻底关闭弹窗；`force` 用于面板自身流程强制关闭不可关闭面板 | 事件选项执行后、Esc 键关闭、赫斯提亚剧情结束链入下一弹窗 |
...
- 事件队列系统（EventSystem）在外部触发事件时，将事件推入 `_eventQueue`；当前设计下随机事件不在中途弹窗，统一由每日结算面板展示（`openSettlementEvent` 用 `push` 入栈，可逐个处理）
- 所有 `open()` 请求经 PopupManager 的待显示队列串行展示，事件弹窗与其他来源的弹窗互不冲掉
