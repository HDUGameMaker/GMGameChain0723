...
时间显示的左侧固定展示人口状态，格式为 `👥当前/住宅/食物`：

| 行为 | 说明 |
|------|------|
| 显示内容 | `👥 8 / 12 / 15` = 当前人口 8、住宅上限 12、食物上限 15 |
| 颜色变化 | 当前人口 == 住宅上限 → 住宅数字变**橙色**（住宅瓶颈）；当前人口 == 食物上限 → 食物数字变**橙色**（食物瓶颈）；两者同时达到 → 都变橙色 |
| 人口变化 | 人口增长/减少时数字带弹跳动画（GSAP），提醒玩家注意人口流动 |
| 查看详情 | **点击**人口数字 → 弹出 tiny popover 显示完整信息：「当前人口 8」「住宅提供 12（工棚×2 + 木板房×5 + 瓦房×?）」「口粮支撑 15（狩猎小屋×1 + 农田×1）」；再次点击关闭 |
| 瓶颈提示 | 当前人口达到住宅或食物上限时，对应数字变橙色。当目标人口 < 当前人口（即将衰减）时计划增加黄色闪烁警告和倒数天数显示 `[计划中]` |
...
| `population.foodFloor` | `number` | 食物人口下限（即使无任何食物建筑，也保证至少支撑 5 人） |
...
| `foodCapacity` | `number?` | 提供的食物人口上限（农场/狩猎类） |
...
人口由「住宅」和「食物」两个独立的上限共同约束，实际人口朝两者中**较小的那个**增长或减少。食物不是资源，不产生、不消耗、不存储——它纯粹是建筑提供的人口容量。

```
居住人口上限 = Σ 所有住宅类建筑的 housingCapacity
食物人口上限 = max(foodFloor, Σ 所有已分配工人的食物建筑的 foodCapacity)

目标人口 = min(居住人口上限, 食物人口上限)
```

> 食物建筑（狩猎小屋、农田）需要分配工人才能提供 `foodCapacity`。未分配工人的食物建筑不贡献食物人口上限。

#### 增长与减少

| 条件 | 行为 |
|------|------|
| 当前人口 < 目标人口 | 每天随机增长 `growthPerDay` 人（如 4~8 人） |
| 当前人口 > 目标人口 | 等待 `declineDelayDays` 天（默认 2 天），之后以相同随机速率减少，直到等于目标人口 |
| 当前人口 = 目标人口 | 不变 |

> **食物下限**：`foodFloor = 5`，即使没有任何食物建筑，食物人口上限最低为 5，保证开局初期不会立刻掉人。
