...
export function renderCultureTreePanel(data, body, pm) {
  ...
  const income = system.getPointIncomeBreakdown?.() || { passive: 0, workforce: 0, total: 0, ruinMultiplier: 1 };
  ...
  top.innerHTML = `<div><b style="color:#dfb9e8">文化树 · ${selectedEra.name}</b><div style="font-size:11px;color:#999">制度、公共生活与文明认同</div></div><div style="text-align:right"><div style="font-size:16px">人文点：<b>${Math.floor(system.getCivicPoints())}</b></div><div style="font-size:10px;color:#c8a7d2">+${income.total.toFixed(1)}/tick · 石碑 ×${(income.ruinMultiplier || 1).toFixed(2)}</div></div>`;
  ...
  for (const node of (content.civics || []).filter(item => item.eraId === selectedEra.id)) {
    ...
    if (!done && !active) {
      ...
      button.addEventListener('click', () => { if (!system.startResearch(node.id)) pm.alert(system.canStartResearch(node.id).reason); renderCultureTreePanel({ eraId: selectedEra.id }, body, pm); });
