...
export function renderSettingsPanel(data, body, pm) {
  ...
  // ===== 新手教程按钮 =====
  const questSys2 = window.__game?.systems?.quest;
  if (questSys2) {
    const questRow = document.createElement('div');
    questRow.style.cssText = 'display:flex;align-items:center;justify-content:space-between;margin-top:10px;';
    const questLabel2 = document.createElement('span');
    questLabel2.style.cssText = 'font-size:13px;color:#a0a0ba;';
    questLabel2.textContent = '📋 新手教程任务';
    const questBtn2 = document.createElement('button');
    questBtn2.textContent = '启动';
    questBtn2.style.cssText = 'padding:6px 16px;border-radius:20px;border:1px solid rgba(91,141,239,0.3);background:rgba(91,141,239,0.15);color:#5b8def;font-size:13px;font-weight:500;cursor:pointer;font-family:inherit;min-width:72px;';
    questBtn2.addEventListener('click', () => {
      questSys2.enable();
      const q = questSys2.getActiveQuest();
      if (q) {
        pm.close();
        setTimeout(() => pm.open('quest_panel', { quest: q }), 200);
      }
    });
    questRow.appendChild(questLabel2);
    questRow.appendChild(questBtn2);
    displaySection.appendChild(questRow);
  }
  ...
  if (window.__cheatManager?.isEnabled()) {
    ...
    enemyLabel.style.cssText = 'font-size:12px;color:#a0a0ba;margin-top:12px;margin-bottom:8px;';
    enemyLabel.textContent = '🦎 敌人控制';
    cheatSection.appendChild(enemyLabel);
    ...
    enemyTypes.forEach(et => {
      ...
      btn.style.cssText = [
        'padding:6px 12px;border-radius:14px;border:1px solid rgba(204,68,68,0.4)',
        ';background:rgba(204,68,68,0.1);color:#ff6b6b',
        ';font-size:12px;font-weight:500;cursor:pointer',
        ';font-family:inherit;transition:all 0.2s;'
      ].join('');
      btn.addEventListener('click', () => {
        const g = window.__game;
        if (!g || !g.systems.combat) return;
        // 如果已经处于放置模式则退出，否则进入
        if (g.systems.combat.isPlaceEnemyMode()) {
          g.systems.combat.exitPlaceEnemyMode();
          btn.textContent = `📍 ${et.name}`;
        } else {
          g.systems.combat.enterPlaceEnemyMode(et.id);
          pm.close();
        }
      });
    ...
    cancelBtn.style.cssText = [
      'padding:6px 12px;border-radius:14px;border:1px solid rgba(255,255,255,0.2)',
    ...
    cancelBtn.addEventListener('click', () => {
      const g = window.__game;
      if (g && g.systems.combat) g.systems.combat.exitPlaceEnemyMode();
    });
    enemyRow.appendChild(cancelBtn);

    const clearBtn = document.createElement('button');
    clearBtn.textContent = '🗑️ 删除所有敌人';
    clearBtn.style.cssText = [
      'padding:6px 12px;border-radius:14px;border:1px solid rgba(255,107,107,0.4)',
    ...
    clearBtn.addEventListener('click', () => {
      const g = window.__game;
      if (!g || !g.systems.combat) return;
      const count = g.systems.combat.enemies.length;
      g.systems.combat.enemies = [];
      g.systems.combat._notify();
      if (g.mapRenderer) g.mapRenderer._drawEnemies();
    });
    enemyRow.appendChild(clearBtn);
    cheatSection.appendChild(enemyRow);
