...
export function renderOutpostDiplomacyPanel(data, body, pm) {
  ...
  const summary = system?.getDiplomaticSummary(data.outpostId);
  if (!system || !outpost || !state || !summary) {
    body.innerHTML = '<div style="padding:36px;color:#808098;text-align:center">城邦数据不可用。</div>';
    return;
  }
  ...
  body.innerHTML = `
    ...
    <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:8px;margin-bottom:16px;font-size:12px;color:#c8c8d6">
  ...
  attack.textContent = state.status === 'defeated' ? '城邦已经被征服' : eligible?.power ? `进攻城邦（最强可用军团 ${eligible.power} / 防御 ${summary.defense}）` : `需要编成${outpost.domain === 'naval' ? '海军' : '陆军'}军团`;
  ...
  tradeLuxury.addEventListener('click', () => pm.push('luxury_trade', { outpostId: outpost.id }));
