...
function renderTree({ body, pm, system, eraId }) {
  ...
  for (const node of nodes) {
    ...
    card.innerHTML = `<img src="${node.icon || ''}" alt="${node.name}" style="width:42px;height:42px;object-fit:contain;align-self:center" onerror="this.style.visibility='hidden'"><b style="text-align:center;color:${done ? '#8ed39c' : '#ead8ae'}">${node.name}</b><span style="font-size:10px;color:#8f93a0;text-align:center">${node.pointCost} 科技点 · ${node.researchTime} tick</span><span style="font-size:10px;color:#e2b86d">研究物资：${resourceCost}</span><span style="font-size:11px;color:#c9c5b8">${node.description || ''}</span><span style="font-size:10px;color:#938d80">${node.history || ''}</span><span style="font-size:10px;color:#8fc59c">效果：${formatResearchEffectsText(node.effects)}</span><span style="font-size:10px;color:#8db9db">解锁：${unlocks}</span><span style="font-size:10px;color:#aaa;flex:1">${done ? '已研究完成' : active ? `研究中 ${progress}/${node.researchTime}` : can.reason || '可以研究'}</span>`;
    if (!done && !active) {
