...
export function formatBuildingEffect(key, value) {
  const names = {
    ...
    blocksEnemyMovement: '阻挡敌军移动', buildSpeedMul: '建造速度', cavalryTrainingMul: '骑兵训练速度', civicPerWorker: '每名工人人文产出', civicPointMul: '人文点数',
    civilizationId: '所属文明', civilizationYieldMul: '文明特色产出', commandPointsBonus: '指挥点', defensePower: '防御力', expeditionAccess: '组织探险',
    ...
    navalPowerMul: '海军战力', navalSpeedMul: '海军移动速度', navalSupply: '海军补给', navalTrainingMul: '海军训练速度', navalVisionRadius: '海上视野半径',
    productionMul: '生产效率', rangedTrainingMul: '远程训练速度', relationGainBonus: '外交关系收益', repairMul: '维修效率', replaces: '替代建筑',
...
export function getBuildingPrimaryFunctionRows(config = {}, getResourceName = id => id) {
  ...
  if (Number(config.maxWorkers) > 0) rows.push(`岗位上限 ${config.maxWorkers} 人`);
  return [...new Set(rows)];
}
